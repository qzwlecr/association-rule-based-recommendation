...
package AR {
  ...
  object Test {
    def main(args: Array[String]): Unit = {
      ...
      val transactions = transactionsFull.sample(false, 0.01, 810L).cache()
      val model = new FPGrowth().setMinSupport(0.092).setNumPartitions(900).run(transactions)

      model.freqItemsets.map(x => x.items.reverse.mkString(" ")).sortBy(x => x).saveAsTextFile(fileOutput + "/freqList_sample_std.dat")
